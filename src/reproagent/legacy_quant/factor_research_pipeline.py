...
class StrategyBacktester:
    def run(self, factor: pd.Series, price: pd.Series, n_groups: int = 5) -> pd.Series:
        ...
        signal[groups == groups.max()] = 1.0
        signal[groups == groups.min()] = -1.0
...
class DeviationAnalyzer:
    def analyze(self, pnl: pd.Series, bench: pd.Series) -> Dict[str, float]:
        diff = (
            pd.concat([pnl.rename("p"), bench.rename("b")], axis=1).fillna(0)["p"]
...
class FactorDiscoverer:
    ...
    def discover(
        self, formulas: List[str], data: pd.DataFrame, price_col: str = "close"
    ) -> List[FactorResult]:
...
class FactorErrorManager:
    ...
    def prepare_params(self, case: DeviationCase) -> Dict[str, Any]:
        if case.category == "entity_merge":
            return {"mode": "merge", "src": "法人库", "dst": "工程线"}
        if case.category == "layout_reflection":
            n = self.max_layout_retries
            return {"mode": "layout", "max_retries": n, "limit": n + 2}
        return {"mode": "unknown"}

    def submit(self, case: DeviationCase) -> DeviationCase:
        case.params = self.prepare_params(case)
        self.tickets.append(case)
        return case
...
class DeviationController:
    ...
    def handle(
        self, case_id: str, is_reverse_pv: bool, category: str, retry_count: int = 0
    ) -> Dict[str, Any]:
        if not is_reverse_pv:
            entry = {"case_id": case_id, "action": "skip", "reason": "not_reverse_pv"}
            self.log.append(entry)
            return entry
        case = DeviationCase(
            case_id=case_id, is_reverse_pv=True, category=category, retry_count=retry_count
        )
        if category == "entity_merge":
            ...
            if retry_count > self.manager.max_layout_retries + 2:
                entry = {"case_id": case_id, "action": "drop", "reason": "exceed_N_plus_2"}
                self.log.append(entry)
                return entry
            ticket = self.manager.submit(case)
        else:
...
if __name__ == "__main__":
    ...
    for f in factors:
        pnl_list = f.pnl.fillna(0).tolist()
        ic_arr = np.array(pnl_list)
        ...
        mdd = float(
            (np.minimum.accumulate(exp_cum) / np.maximum.accumulate(exp_cum) - 1).min() * 100
        )
        win = float((np.array(pnl_list) > 0).mean() * 100)
        ic_data = (np.random.randn(n) * 0.03 + f.ic).clip(-0.35, 0.35).tolist()
        db.save_factor(
            f.name, f.rank, f.ic, f.rank_ic, ic_std, icir, ann, mdd, win, ic_data, pnl_list
        )
        print(f"  + {f.name} rank={f.rank} ic={f.ic:.4f} -> DB saved")
    ctrl = DeviationController(max_layout_retries=2)
    for cid, rev, cat, retry in [
        ("C-001", True, "entity_merge", 0),
        ("C-002", True, "layout_reflection", 1),
        ("C-003", False, "entity_merge", 0),
    ]:
        r = ctrl.handle(cid, rev, cat, retry)
        db.save_deviation(
            r.get("case_id", cid), r.get("action", "?"), r.get("category", ""), r.get("params", {})
        )
