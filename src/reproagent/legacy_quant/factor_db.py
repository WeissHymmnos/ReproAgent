...
class FactorDB:
    ...
    def save_factor(self, name: str, rank: int, ic_mean: float, rank_ic: float,
                    ic_std: float, icir: float, ann_return: float,
                    max_drawdown: float, win_rate: float,
                    ic_series: List[float], pnl_series: List[float]) -> int:
    ...
    def seed_demo(self):
        ...
        for i, name in enumerate(["动量因子", "价值因子", "质量因子", "成长因子", "低波因子",
                "规模因子", "红利因子", "情绪因子", "技术因子", "流动性因子",
                "盈利因子", "杠杆因子"]):
            ...
            s = float(np.mean(ic)); st = float(np.std(ic))
            cum = np.exp(np.cumsum(dr))
            running_max = np.maximum.accumulate(cum)
            mdd = float(((cum - running_max) / running_max).min() * 100)
            self.save_factor(name, i + 1, s, float(np.corrcoef(ic, np.argsort(dr))[0, 1]) if len(ic) > 1 else 0,
                             st, s / st if st > 0 else 0,
                             round((np.exp(np.cumsum(dr))[-1] - 1) * 100, 2),
                             round(mdd, 2), round(float((dr > 0).mean() * 100), 2), ic, dr.tolist())
        self.save_deviation("C-001", "submit", "entity_merge", {"mode":"merge","src":"法人库","dst":"工程线"})
        self.save_deviation("C-002", "submit", "layout_reflection", {"mode":"layout","max_retries":2,"limit":4})
