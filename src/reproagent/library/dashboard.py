...
def generate_html_dashboard(
    ...
    """生成因子库 HTML 仪表盘，返回输出路径。

    factors: list[dict]，每项形如::
        {"name": str, "ic_series": list[float], "excess_cum": list[float],
         "stats": {"ic": float, "icir": float, "ann_return": float,
                   "max_drawdown": float, "win_rate": float, "std": float}}
    """
...
<div class="toolbar">
  ...
  <div class="count" id="visibleCount">显示 {count} / {count} 个</div>
</div>
