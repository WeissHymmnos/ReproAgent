...
from finreportparser.fusion.headers_footers import strip_header_footer_lines
...
def strip_repeated_headers_footers(text: str) -> str:
    """Legacy name — delegates to shared header/footer line stripper + table scrub."""
    if not text:
        return text
        ...
        if stripped.startswith("|") and stripped.endswith("|"):
            ...
            for i, cell in enumerate(cells):
                ...
                if cell_stripped == "HAITONG":
                    ...
                    cleaned_cells.append(" " * len(cell))
                elif cell_stripped in ["金融工程研究", "证券研究报告", "金融工程专题报告", "固收研究"]:
                    cleaned_cells.append(" " * len(cell))
                elif cell_stripped.startswith("请务必阅读正文之后"):
                    cleaned_cells.append(" " * len(cell))
                elif cell_stripped.startswith("免责声明和披露"):
                    cleaned_cells.append(" " * len(cell))
    ...
    joined = "\n".join(cleaned_lines)
    return strip_header_footer_lines(joined)
